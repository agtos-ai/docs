...
<Accordion title="GET /health/metrics">
...
<Accordion title="GET /health/:serviceName">
...
<ParamField path="serviceName" type="string" required>
  Service name (e.g., `redis`, `ollama`, `stt-sherpa-onnx`, `tts-sherpa-onnx`, `mcp-server`).
</ParamField>
...
<ParamField body="provider" type="string" required>
  Provider identifier: `provider-anthropic` or `provider-openai`.
</ParamField>

<ParamField body="apiKey" type="string">
  API key (e.g., `sk-ant-api03-...` for Anthropic, `sk-...` for OpenAI). Max 256 chars.
</ParamField>

<ParamField body="setupToken" type="string">
  OAuth setup token (e.g., `sk-ant-oat01-...` for Anthropic Max subscription). Max 256 chars.
</ParamField>
...
<ParamField body="setupToken" type="string">
  OAuth token to validate. Max 256 chars.
</ParamField>
...
<Accordion title="POST /api/dependencies/install-redis">
...
Possible `status` values: `installed` (new container created), `started` (existing container started), `already_running` (no action needed).

**400 Bad Request** -- Docker is not running.
...
- `POST /api/credentials` (localhost-only, needed during onboarding setup)
...
| Chat, Tasks, Credentials (`/api/chat`, `/api/tasks`, `/api/credentials`) | 20 requests/min | `CHAT_RATE_LIMIT` |
