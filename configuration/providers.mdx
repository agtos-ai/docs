...
[speaches](https://github.com/speaches-ai/speaches) is a self-hosted server that provides OpenAI-compatible speech-to-text and text-to-speech endpoints. agtOS uses it for the cascade voice pipeline.
...
sherpa-onnx runs STT, TTS, and VAD directly in the Node.js process via a native ONNX Runtime addon. No Python, no external server, no HTTP round-trips. See [ADR-017](/architecture/decisions#adr-017) for the decision rationale.
...
```bash
# Enable sherpa-onnx (replaces speaches)
