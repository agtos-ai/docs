...
description: "Configure Claude, OpenAI, Ollama, sherpa-onnx, speaches, and the model router"
---
...
OpenAI is an alternative cloud LLM provider for agtOS, available as a drop-in replacement for Claude in the model router's cloud tier ([ADR-019](/architecture/decisions#adr-019)). It uses the OpenAI Node SDK v6 with streaming support.
...
# Enable OpenAI as the cloud provider
AGTOS_CLOUD_PROVIDER=openai
...
<Note>
When `AGTOS_CLOUD_PROVIDER=openai`, the model router sends all Tier 3 (cloud) requests to OpenAI instead of Claude. If the OpenAI provider fails to initialize, it falls back to Claude with a warning.
</Note>
...
# Cloud provider selection (claude or openai)
AGTOS_CLOUD_PROVIDER=claude
...
| `cloudProvider` | `claude` | Cloud tier provider: `claude` or `openai` (via `AGTOS_CLOUD_PROVIDER`) |
...
<CardGroup cols={2}>
  ...
  <Card title="OpenAI (Cloud)" icon="cloud">
    Alternative cloud provider. GPT-4o for reasoning, GPT-4o Mini for speed. Selectable via `AGTOS_CLOUD_PROVIDER=openai`.
  </Card>
